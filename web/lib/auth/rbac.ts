...
const ADMIN_SUBSECTION_CAPABILITY: Array<[string, keyof CapabilitySet]> = [
  ...
  // Exact fee amounts live here, so this is gated on the payment capability specifically rather
  // than falling through to the generic canViewAllAssets check every other /admin/* page not
  // listed above gets — several roles other than payment_admin/admin have that capability too.
  ["/admin/payments", "canMarkPaymentDone"],
];
