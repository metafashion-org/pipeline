...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ draftId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: errorMessage(error, "Failed to save draft") }, { status: 400 });
  }
}
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ draftId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: errorMessage(error, "Failed to discard draft") }, { status: 400 });
  }
}
